...
function RealForm({ onNewReal }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/real', formData);
    ...
    <div className='real-form-container flex justify-center items-center h-full mt-20'>
      <div className="w-full max-w-sm p-4 bg-white rounded-md shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input type="text" name="image" placeholder="Image URL" value={formData.image} onChange={handleChange} className="border border-gray-300 px-3 py-2 rounded-md mb-2 block w-full" />
          <input type="text" name="pdf" placeholder="PDF URL" value={formData.pdf} onChange={handleChange} className="border border-gray-300 px-3 py-2 rounded-md mb-2 block w-full" />
